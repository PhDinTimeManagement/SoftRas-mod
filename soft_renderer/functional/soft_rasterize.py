...
class SoftRasterizeFunction(Function):
    ...
    def forward(ctx, face_vertices, textures, image_size=256,
        ...
        ctx.dist_eps = float(np.log(1. / dist_eps - 1.))
        ...
        faces_info, aggrs_info, soft_colors = soft_rasterize_cuda.forward_soft_rasterize(
            face_vertices,
            textures,
            faces_info,
            aggrs_info,
    ...
    def backward(ctx, grad_soft_colors):
        ...
        grad_faces = torch.zeros_like(face_vertices, dtype=face_vertices.dtype, device=ctx.device)
        grad_textures = torch.zeros_like(textures, dtype=textures.dtype, device=ctx.device)
        grad_soft_colors = grad_soft_colors.contiguous()

        grad_faces, grad_textures = soft_rasterize_cuda.backward_soft_rasterize(
            face_vertices,
            textures,
            soft_colors,
            faces_info,
...
def soft_rasterize(face_vertices, textures, image_size=256,
                   ...
                   gamma_val=1e-4, aggr_func_rgb='softmax', aggr_func_alpha='prod',
                   texture_type='surface'):
    if face_vertices.device.type == 'cpu':
        raise TypeError('Rasterize module supports only cuda Tensors')
    if textures.device != face_vertices.device:
        raise TypeError('face_vertices and textures must be on the same device')
    if textures.dtype != face_vertices.dtype:
        raise TypeError('face_vertices and textures must have the same dtype')
